...
//    // TODO 26 добавьте два rassert-а - чтобы удостовериться что размеры двух динамических массивов которые мы только что изменили - совпадают с вашим ожиданием
    rassert(row1.size()==3, "2_3")
    rassert(array2d.size()==2, "2_4")
    std::cout << "print2DArray(array2d):" << std::endl;
...
void task3() {
    ...
    rassert(rows<=20&&rows>=1, "3_0")
    rassert(cols<=20&&cols>=1, "3_0")
    ...
    while (true) {
        ...
        for(int i = 0; i < array2d.size(); i++){
            bool rows_count = true;
            for(int j = 0; j < array2d[i].size(); j++){
                if(array2d[i][j] != 1){rows_count = false;}
            }
            if(rows_count){std::cout<<"OX-XO-XO"<<std::endl; break;}
        ...
        for(int j = 0; j < array2d[0].size(); j++){
            bool cols_count = true;
            for(int i = 0; i < array2d.size(); i ++){
                if(array2d[i][j] != 1){cols_count = false;}
            }
            if(cols_count){std::cout<<"AX-XA-XA"<<std::endl; break;}
